...
class DomainStore(SQLiteStore):
    ...
    def save_fact(self, fact):
        ...
        if not fact.fact_id.strip() or not fact.statement.strip(): raise ValueError("invalid fact")
    ...
    def save_analysis(self, analysis):
        if not analysis.analysis_id.strip() or not analysis.source_fact_ids: raise ValueError("invalid analysis")
        if not 0 <= analysis.confidence <= 1: raise ValueError("analysis confidence must be between 0 and 1")
    ...
    def save_knowledge(self, knowledge, *, actor: str, reason: str = ""):
        ...
        try:
            ...
            self.connection.execute("INSERT INTO knowledge VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(knowledge_id) DO UPDATE SET source_fact_ids=excluded.source_fact_ids,source_analysis_ids=excluded.source_analysis_ids,statement=excluded.statement,evidence_count=excluded.evidence_count,validation_status=excluded.validation_status,validated_by=excluded.validated_by,validated_at=excluded.validated_at,valid_until=excluded.valid_until,security_classification=excluded.security_classification,evidence_diversity=excluded.evidence_diversity,superseded_by=excluded.superseded_by,status_changed_at=excluded.status_changed_at,status_changed_by=excluded.status_changed_by", (knowledge.knowledge_id,self._json(knowledge.source_fact_ids),self._json(knowledge.source_analysis_ids),knowledge.statement,knowledge.evidence_count,knowledge.validation_status,knowledge.validated_by,knowledge.validated_at.isoformat() if knowledge.validated_at else None,knowledge.valid_until.isoformat() if knowledge.valid_until else None,knowledge.security_classification.value,knowledge.evidence_diversity,knowledge.superseded_by,knowledge.status_changed_at.isoformat() if knowledge.status_changed_at else None,knowledge.status_changed_by))
